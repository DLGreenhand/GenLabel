...
const intervalScreenshot = (page, url, abSavePath, interval, times) => new Promise(async (resolve, reject) => {
    let cnt = -1;
    await page.goto(url.address, { waitUntil: 'networkidle0', timeout: 0 }).catch(err => reject(err));
    ...
    const intervalId = setInterval(async () => {
        ...
        if (cnt >= times) {
            ...
            for (let i = 0; i < times && flag; i++) {
                flag = flag && fs.existsSync(`${abSavePath}/u_${`${url.id}`.padStart(5, "0")}_${`${times - 1}`.padStart(3, "0")}.png`);
            }
            if (!flag) return;
            clearInterval(intervalId);
            resolve();
        } else {
            await page.screenshot({ path: `${abSavePath}/u_${`${url.id}`.padStart(5, "0")}_${`${cnt}`.padStart(3, "0")}.png` }).catch(err => reject(err));
        }
    }, interval)
})
...
const pageScreenshot = (batch, browser, urls, abSavePath, interval = 1000, times = 5) => new Promise(async (resolve) => {
    ...
    for (let i = 0; i < urls.length; i++) {
        await intervalScreenshot(page, { id: batch + i, address: urls[i] }, abSavePath, interval, times).catch((err) => {
            console.log(err, `\n\nERR: [${batch + i}] ${urls[i]}\n\n`);
